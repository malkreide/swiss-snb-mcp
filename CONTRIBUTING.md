...
**Cadence:** daily at 03:17 UTC, plus on demand via *Actions → CI → Run
workflow*. See [`.github/workflows/ci.yml`](.github/workflows/ci.yml).

**Who sees it:** A red run opens an issue labelled `upstream` and the stable title “Live-Tests gegen data.snb.ch rot (<Datum>)”. A second red run recognises the open issue by its title prefix and appends to that same thread rather than opening a second one. Once the suite is green again, the issue closes itself.

**Three answers, not two.** `scripts/classify_live_scenarios.py` reads the JUnit XML rather than
...
wrong assumption about data.snb.ch. Every other test asserts against a fixture, and
