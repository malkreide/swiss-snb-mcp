...
USER_AGENT = f"swiss-snb-mcp/{__version__} (+https://github.com/malkreide/swiss-snb-mcp)"
...
async def _lifespan(server: MCPServer) -> AsyncIterator[None]:
    ...
    _runtime.http = httpx.AsyncClient(timeout=DEFAULT_TIMEOUT, follow_redirects=False, headers={"User-Agent": USER_AGENT})
    try:
        yield
    finally:
